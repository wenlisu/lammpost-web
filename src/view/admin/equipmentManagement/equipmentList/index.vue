...
<template>
  <div id="equipmentList">
    <div v-if="routeName == 'equipmentListDetail'">
      <Detail />
    </div>
    <div v-if="routeName == 'equipmentList'">
      <div class="flexCloumn equipmentStatistics">
        <el-select
          v-model="statisticsProduct"
          placeholder="请选择"
        ...
        <div class="flexRow info">
          <div class="flexRow">
            ...
            <div class="flexCloumn infoDetail">
              ...
              <div class="number">
                <span>{{ statistics.totalDevice }}</span
                >个
              </div>
            </div>
          </div>
          <div class="flexRow">
            ...
            <div class="flexCloumn infoDetail">
              ...
              <div class="number">
                <span>{{ statistics.activateDevice }}</span
                >个
              </div>
            </div>
          </div>
      ...
      <!-- 设备管理 -->
      <header-title icon="iconfont icon-shouye" title="设备管理">
        <search-form
          slot="searchBox"
          :searchItems="searchItems"
          @onSearch="onSearch"
          @onChange="onChange"
          :searchChangeValue="searchChangeVal"
          :addBtnVisible="true || $_hasOperation($options.name, 'add')"
          @on-addBtn="showView({ type: 'add' })"
          :delBtnVisible="true"
          @on-delBtn="showView({ type: 'del' })"
          :otherBtnVisible="true"
          :otherBtnLists="[
            {
              label: '注册设备',
              disabled: selectsList.length === 0,
              click: () => showView({ type: 'register' }),
            },
          ]"
        />
      </header-title>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      tableCols: [
        ...
        { label: "设备所属产品", prop: "productName" },
        { label: "厂商名称", prop: "factoryName" },
        { label: "设备类型", prop: "deviceName" },
        { label: "设备型号", prop: "deviceName" },
        { label: "设备版本", prop: "version" },
        { label: "激活状态", prop: "activateStatusText" },
        { label: "设备关联状态", prop: "bindStatusText", minWidth: "110px" },
      ...
      formRules: {
        sensorTypeCode: [
          {
            required: true,
            validator: validateSensorTypeCode,
            trigger: "change",
          },
        ],
        manufacturerCode: [
          {
            required: true,
            validator: validateManufacturerManageCode,
            trigger: "change",
          },
        ],
      },
  ...
  methods: {
    ...
    async onChange(val) {
      if (
        val.manufacturerManageCode !== this.searchChangeVal.manufacturerManageCode ||
        val.sensorTypeCode !== this.searchChangeVal.sensorTypeCode
      ) {
    ...
    getDataAllList() {
      let data = {
        data: {
          ...this.searchVal,
          productNo:
            this.statisticsProduct !== "-1" ? this.statisticsProduct : null,
        },

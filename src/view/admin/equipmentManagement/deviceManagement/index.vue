...
import {
  getDeviceTypeManagementPage,
  deleteDeviceTypeManagement,
  addDeviceTypeManagement,
  updateDeviceTypeManagement
} from 'api/admin';
import {
...
export default {
  ...
  methods: {
    ...
    getDataAllList () {
      ...
      getDeviceTypeManagementPage(data).then(res => {
        if (res.code === '200') {
          const {
            data,
            total
    ...
    handleDel (item) {
      this.$confirm('此操作将永久删除该设施类型, 是否继续?', '提示', {
        ...
        deleteDeviceTypeManagement(parames)
          .then(res => {
            if (res.code === '200') {
              this.$notify({
                title: '成功',
    ...
    async dialogSubmit (val) {
      let apiMethod = addDeviceTypeManagement;
      let message = '新增成功';

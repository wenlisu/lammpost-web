<template>
  ...
  >
    <div
      v-if="status == 1"
      @click="handleClick(allInfo.deviceIndex);"
    >
      <layer />
    </div>
    <div
    ...
    <div class="info-layer" v-if="show">
      <div class="header">
        ...
        <div class="detai">
          <p class="name">{{allInfo['deviceName'] || ''}}</p>
          <p class="addr">{{allInfo['location'] || ''}}</p>
        </div>
      </div>
      <div class="content">
        <el-row :gutter="10">
          <el-col :span="24">
            <span>设备型号：{{allInfo['deviceIndex'] || ''}}</span>
          </el-col>
        </el-row>
        <el-row :gutter="10">
          <el-col :span="12">
            <p>
              设备状态：<span :class="{ status: true, alarm: status == 3 }">{{
                status == 1 ? "在线" : "离线"
              }}</span>
            </p>
          </el-col>
          <el-col :span="12">
            <p>
              LED状态：<span :class="{ status: true, alarm: allInfo['led'] == 0 }">{{
                allInfo['led'] == 1 ? "开启" : "关闭"
              }}</span>
            </p>
          </el-col>
        ...
        <el-row :gutter="10">
          <el-col :span="24">
            <span>IP地址：{{allInfo['deviceIp'] || ''}}</span>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="24">
            <span>MAC地址：{{allInfo['deviceMac'] || ''}}</span>
          </el-col>
        </el-row>
      </div>
    </div>
...
import { queryWifiInfoById } from "api/wifi";
...
export default {
  ...
  created() {
    // this._queryWifiInfoById(this.allInfo.deviceIndex)
  },
  methods: {
    _queryWifiInfoById(id) {
      const params = {
        deviceId: id
      }

  ...
  <div class="warm-map-box">
    <div class="map">
      ...
      >
        ...
        <div class="tipIcon">
          <div class="normal">
            <img src="./images/online_icon.png" alt="" srcset="" />
            <span>在线</span>
          </div>
          <div class="alarm">
            <img src="./images/offline_icon.png" alt="" srcset="" />
            <span>离线</span>
          </div>
        </div>
      </baidu-map>
...
import { getAudioDeviceMapInfo, getAudioPlayCountRank } from "api/brocast";
...
export default {
  ...
  data() {
    return {
      ...
      linePoint: [
        [
          {
            center: { lng: 113.204103, lat: 23.146474 },
            status: 0,
          },
          {
            center: { lng: 113.2202, lat: 23.055525 },
            status: 1,
          },
          {
            center: { lng: 113.302988, lat: 23.066165 },
            status: 1,
          },
          {
            center: { lng: 113.346107, lat: 23.037966 },
            status: 0,
          },
        ],
        [
        ...
        [
          ...
          {
            center: { lng: 113.151714, lat: 23.121714 },
            status: 1,
          },
          {
            center: { lng: 113.175932, lat: 23.123509 },
            status: 1,
          },
        ],
        [
          {
            center: { lng: 113.170039, lat: 23.109815 },
            status: 1,
          },
          {
            center: { lng: 113.182544, lat: 23.113737 },
            status: 1,
          },
          {
            center: { lng: 113.199145, lat: 23.123642 },
            status: 1,
          },
          {
            center: { lng: 113.201732, lat: 23.108486 },
            status: 1,
          },
        ],
      ],
  ...
  created() {
    // this._getAudioDeviceMapInfo();
  },
  destroyed() {
    this.infoTimer && clearTimeout(this.infoTimer);
  },
  methods: {
    _getAudioDeviceMapInfo() {
      return getAudioDeviceMapInfo().then(async (res) => {
        if (res.code === "200") {
          const { data } = res;
    ...
    async handler({ BMap, map }) {
      ...
      this.BMap = BMap;
      const points = await this._getAudioDeviceMapInfo()
      setTimeout(() => {
        map.setMapStyleV2({
        ...
        // this.myLayer ? (this.myLayer[0].show = true) : null;
        if (this.myLayer) {

  ...
  <div id="lightTable">
    <el-form ref="form" :model="form">
        ...
        >
            ...
            <span class="adress"><p>{{item.deviceName}}</p></span>
            <span class="number">{{getStatus(item)}}</span>
            </li>
        </ul>
        </div>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    createFilter (queryString) {
      return (restaurant) => {
        return (restaurant.deviceName.toLowerCase().indexOf(queryString.toLowerCase()) > -1);
      };
    },
    handleSelect (item) {
      this.form = {

  ...
  <div class="warm-map-box">
    <div class="map">
      ...
      >
        ...
        <div class="tipIcon">
          <div class="normal">
            <img src="./images/lamp_1.png" alt="" srcset="" />
            <span>正常</span>
          </div>
          <div class="alarm">
            <img src="./images/lamp_2.png" alt="" srcset="" />
            <span>报警</span>
          </div>
        </div>
      </baidu-map>
    ...
    <div class="bottom-table">
      <commonTitle title="报警器报告消息"></commonTitle>
      <infoTable />
    </div>
...
import { getDeviceInfo } from '@/api/wisdomWarn';
...
export default {
  ...
  data () {
    return {
      ...
      // 随机坐标
      randomPos: [
        { lng: 113.204103, lat: 23.146474 },
        { lng: 113.2202, lat: 23.055525 },
        { lng: 113.302988, lat: 23.066165 },
        { lng: 113.346107, lat: 23.037966 },
        { lng: 113.168458, lat: 23.140625 },
        { lng: 113.177657, lat: 23.067761 },
        { lng: 113.182831, lat: 22.989535 },
        { lng: 116.728521, lat: 39.838522 },
        { lng: 116.286986, lat: 39.832316 },
        { lng: 116.615263, lat: 39.295644 }
      ],
      allPoint: [], // 所有报警器的经纬度
      active: true,
      road: [], // 道路的起始点
  ...
  methods: {
    getPointData () {
      ...
      return getDeviceInfo(params).then(async (res) => {
        if (res.code === '200') {
          const { data } = res.data;
          this.allPoint = await Promise.all(data.map((e) => {
            let radom = Math.floor(Math.random() * 10);
            return {
              ...e,
              center: {
                lng: e.longitude || this.randomPos[radom].lng,
                lat: e.latitude || this.randomPos[radom].lat
              },
              status: e.alarmStatus
            };
          })
          );
    ...
    async handler ({ BMap, map }) {
      ...
      // });
      const infos = await this.getPointData();
      if (infos.length > 0) {
        let startP = infos[1].center;

...
export default {
    ...
    currentVal: {
      get () {
        if (this.value instanceof Array) {
          const reg = /[\u4e00-\u9fa5]/g;
          const val = this.value.length > 0 ? this.value.filter(v => {
            if (!!v.imei && !reg.test(v.imei) && !!v.serialNumber) {
              return v
            }
          }) : null;
          this.setDom(val);
      ...
      set (newVal) {
        if (newVal instanceof Array) {
          const reg = /[\u4e00-\u9fa5]/g;
          const val = newVal.filter(v => (!!v.imei && !reg.test(v.imei)) || (!!v.serialNumber && !reg.test(v.serialNumber)));
  ...
  mounted () {
    this.$nextTick(async () => {
      this.options = await this.loadAll({});
    });
  },
  methods: {
    setDom (val) {
    ...
    loadAll (parames) {
      return getTerminalTree({...parames, deviceTypeNo: 'AUDIO'}).then(async res => {
        if (res.code === '200') {
          let dataList = [];
          res.data.map(async data => {
            ...
            if (!data.serialNumber) {
              ...
              if (!data.deviceModuleCount && paramesData.imei) {
                delete paramesData.imei;
              }
              addressChildren = await this.loadAll(paramesData);
            } else {
              addressChildren = null
            }
            const id = data.deviceModuleCount && data.imei ? data.terminalName : (data.serialNumber || data.address || data.area || data.city);
            const label = data.deviceModuleCount && data.imei ? data.terminalName : (data.deviceModuleName || data.address || data.area || data.city)
            dataList.push({
              ...parames,
              ...data,
              id,

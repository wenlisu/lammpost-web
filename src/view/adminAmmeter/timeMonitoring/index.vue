<template>
  <div id="timeMonitoring">
    ...
    <div class="flexCloumn timeMonitoringSelect">
      ...
      <div class="flexCloumn loor" v-if="monitorId === 'floor'">
        ...
        <div class="flexRow loorInfo">
          ...
          <div>
            ...
            <div class="flexCloumn info">
              <span>{{floorInfo.info.warningCount}}</span>
              <span>未缴费</span>
            </div>
          </div>
        </div>
        <div class="flexCloumn loorDeviceList">
          <span class="infoTitle">本楼层电表情况</span>
        ...
        <el-tabs v-model="activetLoorTabsName" type="card" @tab-click="handleLoorTabClick">
          ...
          <el-tab-pane
            ...
            lazy
          >
            <span slot="label"><i class="iconfont icon-chanpinxinxi"></i> 抄表记录</span>
            <div class="content">
              <page-table
                col-align="center"
      ...
      <div class="flexCloumn alarm" v-if="monitorId === 'device'">
      ...
      <div class="flexCloumn normal" v-if="monitorId === 'normal'">
        <div class="flexRow normalHeader">
          ...
          <div class="flexRow fastInfoAction">
            <!-- <div class="flexRow batchWater">
              <div class="icon"><i class="iconfont icon-dianji" /></div>
              远程抄表
            </div> -->
        ...
        <el-tabs v-model="activetNormalTabsName" type="card" @tab-click="handleNormalTabClick">
          <el-tab-pane
            label="基本信息"
            name="1"
            lazy
          >
            ...
            <div class="flexCloumn normalBaseInfo content">
              <div class="flexCloumn">
                <div class="flexCloumn baseInfoHeader">
                  ...
                  <el-row :gutter="10" class="baseItems">
                    <el-col
                    ...
                    >
                      <div class="flexRow content">
                        ...
                        <div
                          ...
                          :style="{ background: item.backgroundColor, boxShadow: item.boxShadow }"
                        >
                ...
                <div class="waterTrends">
                  <div class="flexRow title">
                    <span>72小时用电量趋势分析</span>
                    <div class="flexRow laber">
                      <span />
                      光照强度
                    </div>
                  </div>
                  <div id="waterTrendsChart" class="chart"/>
                </div>
              </div>
            </div>
          </el-tab-pane>
          ...
          >
            ...
            <div class="flexCloumn usersPayInfo content">
              <el-row :gutter="10">
                <el-col :span="12" class="userPayInfo">
                  <div class="flexCloumn itemContent">
                    <div class="flexRow titleHead">
                      <span />
                      用水信息
                    </div>
                    <div class="flexRow address">
                      <span class="addressInfo">用户电表地址：天河区 - 海安路 - 维多利花园 - 10F - 1001室</span>
                      <span class="status">未缴</span>
                    </div>
                    <div class="flexRow item1">
                      <div class="flexCenter icon">
                        <i class="iconfont icon-mingzi" />
                      </div>
                      <div class="flexCloumn item1Info">
                        <div class="title">
                          李四
                        </div>
                        <FormItemText
                          :layout="normalInfo.addressInfoItems"
                          labelWidth="0px"
                        >
                        </FormItemText>
                      </div>
                    </div>
                    <el-row :gutter="10" class="item2">
                      <el-col
                        :span="8"
                        v-for="item in normalInfo.addressInfoItemDoms"
                        :key="item.id"
                        class="item2SunItem"
                      >
                        <div
                          class="flexRow item2SunItemContent"
                          :style="{
                        ...
                        >
                          ...
                          <div class="flexCloumn item2Info">
                            <div class="value">
                    ...
                    <div class="flexRow item3">
                      电费：<span>7488599.45</span> 元
                    </div>
                    <div class="flexRow item3">
                      <div>当前余额：<span>7488599.45</span> 元</div>
                      <div>大写：肆拾玖园零角柒分</div>
                    </div>
                    <div class="flexRow item3">
                      <div>可用电量：<span>89.45</span> 元</div>
                      <div>可用天数：<span>15</span> 天</div>
                    </div>
                ...
                <el-col :span="12" class="water2Trends">
                  <div class="itemContent">
                    ...
                    <div id="water2TrendsChart" class="chart"/>
                  </div>
                </el-col>
              </el-row>
              <div class="payTable">
                ...
                <page-table
                  ...
                  :selection="false"
                  :operator="false"
                  :data="playHistoryList.tableData"
                  :cols="playHistoryList.tableCols"
                  :page-current="playHistoryList.page.current"
                  :total="playHistoryList.page.total"
                  :page-size="playHistoryList.page.pageSize"
                  @current-change="onPlayHistoryListCurrentChange"
                  @size-change="onPlayHistoryListPageSizeChange"
                  @row-current-change="onPlayHistoryListSelectionChange"
                >
                </page-table>
              </div>
            </div>
...
export default {
    ...
    return {
      ...
      normalInfo: {
        ...
        baseInfoItemDoms: [{
          ...
          name: '电压',
          unit: 'm³',
          backgroundColor: 'linear-gradient(135deg, #40E07B 0%, #30E2B0 100%)',
          boxShadow: '0px 2px 6px 0px rgba(205,255,246,1)',
          icon: 'dianji',
          value: '233443.02'
        }, {
          id: 'b',
          name: '电流',
          unit: 'kwh',
          backgroundColor: 'linear-gradient(135deg, #38C9E7 0%, #12D7D6 100%)',
          boxShadow: '0px 2px 6px 0px rgba(205,255,246,1)',
          icon: 'dianji',
          value: '233443.02',
          label: '当日最大值：300m³/h'
          ...
          name: '有功功率',
          unit: 'W',
          backgroundColor: 'linear-gradient(135deg, #7070FD 0%, #509AFF 100%)',
          boxShadow: '0px 2px 4px 0px rgba(210,224,255,1)',
          icon: 'dianji',
          value: '关'
        }, {
          id: 'd',
          name: '通断状态',
          unit: '',
          backgroundColor: 'linear-gradient(135deg, #F9773B 0%, #FF9336 100%)',
          boxShadow: '0px 2px 6px 0px rgba(245,220,179,1)',
          icon: 'dianji',
          value: '正常/异常'
        }],
        addressInfoItems: [{
  ...
  methods: {
    ...
    //   if (node.level === 0) {
    //     this._getWaterMeterSiteTree({
    //     }, 'areaCode', 'areaName', 'area', resolve);
    //   }
    //   if (node.level === 1) {
    //     this._getWaterMeterSiteTree({
    //       areaCode: node.data.areaCode
    //     }, 'streetCode', 'streetName', 'street', resolve);
    //   }
    //   if (node.level === 2) {
    //     this._getWaterMeterSiteTree({
    //       streetCode: node.data.streetCode,
    //       areaCode: node.data.areaCode
    //     }, 'parkCode', 'parkName', 'park', resolve);
    //   }
    //   if (node.level === 3) {
    //     this._getWaterMeterSiteTree({
    //       parkCode: node.data.parkCode,
    //       areaCode: node.data.areaCode,
    //       streetCode: node.data.streetCode
    //     }, 'buildingCode', 'buildingName', 'building', resolve);
    //   }
    //   if (node.level === 4) {
    //     this._getWaterMeterSiteTree({
    //       parkCode: node.data.parkCode,
    //       areaCode: node.data.areaCode,
    //       streetCode: node.data.streetCode
    //     }, 'floorCode', 'floorName', 'floor', resolve);
    //   }
    //   if (node.level === 5) {
    //     getWaterMeterSiteTree({
    //       parkCode: node.data.parkCode,
    //       areaCode: node.data.areaCode,
    //       buildingCode: node.data.buildingCode,
    //       streetCode: node.data.streetCode,
    //       floorCode: node.data.floorCode
    //     }).then(res => {
    //       if (res.code === '200') {
    //         const treeData = res.data.map(daItem => {
    //           return {
    //             id: daItem['serialNumber'],
    //             label: daItem['deviceName'],
    //             type: 'device',
    //             statusText: daItem.onlineStatus === 1 ? alarmStatusType[daItem.alarmStatus] : onlineStatusType[daItem.onlineStatus],
    //             ...daItem,
    //             leaf: true
    //           }
    //         });
    ...
    async loadNode (node, resolve) {
      if (node.level === 0) {
        const treeData = await this._getParkDeviceTree({
          parentNodeCode: '0',
          deviceTypeNo: 'ELECTRICITY_METER'
    ...
    //     this.buildingInfo.name = `${data.areaName}${data.buildingName}`;
    //     getWaterMeterCurrentStatus({
    //       parkCode: data.parkCode,
    //       areaCode: data.areaCode,
    //       streetCode: data.streetCode,
    //       buildingCode: data.id
    //     }).then(res => {
    //       if (res.code === '200') {
    ...
    //     this.floorInfo.name = `${data.floorName}各烟感设备状态`;
    //     getSmokeDetectorCurrentStatus({
    //       parkCode: data.parkCode,
    //       areaCode: data.areaCode,
    ...
    nodeClick (data) {
      ...
      this.currentNodeKey = data.id;
      if (data.id && data.type === 'building') {
        this.buildingInfo.name = `${data.label}电表设备状态`;
        getWaterMeterCurrentStatus({
      ...
      if (data.id && data.type === 'floor') {
        this.floorInfo.name = `${data.label}各电表设备状态`;
    ...
    _getSmokeDetectorDeviceDtl (item, type) {
      this.monitorId = statusPandType[type];
        ...
        getSmokeDetectorDeviceDtl(item.serialNumber).then(async res => {
          if (res.code === '200') {
            ...
            if (this.monitorId === 'normal') {
              ...
              this.normalInfo.statusText = type;
              ...
              this.normalInfo.listItems = [{
                ...
                thresholdValue: data.smokeConcentrationThreshold,
                ...
                thresholdValue: data.temperatureThreshold,
                ...
                name: '湿度',
                value: data.humidity,
                thresholdValue: data.humidityThreshold,
                maxValue: data.humidityMax,
                unit: '%'
              }, {
                ...this.normalInfo.listItems[3],
                id: 'd',
                name: '报警状态',
              ...
              getSmokeDetectorTodayAlarmStats(item.serialNumber).then(chartRes => {
                if (chartRes.code === '200') {
                  ...
                  chartRes.data.map(dchartData => {
                    alarmAnalyze.date.push(dchartData.simpleTime);
              ...
              getSmokeDetectorTodayStats(item.serialNumber).then(chartRes => {
                if (chartRes.code === '200') {
                  ...
                  chartRes.data.map(dchartData => {
                    threeAnalyze.date.push(dchartData.simpleTime);
                    threeAnalyze.smokeConcentration.push(dchartData.smokeConcentration);
                    threeAnalyze.smokeConcentrationThreshold.push(data.smokeConcentration);
                    threeAnalyze.temperature.push(dchartData.temperature);
                    threeAnalyze.temperatureThreshold.push(data.temperatureThreshold);
      ...
      if (this.monitorId === 'alarm') {
        getSmokeDetectorFinalAlarm(item.serialNumber).then(async res => {
          if (res.code === '200') {
            const data = res.data;
            this.alarmInfo.info = {
              ...data,
              disposeSelect: [2, 3, 5, 8, 9]
            };
    ...
    // _getParkDeviceTree (data, id, label, type, resolve) {
    //   getParkDeviceTree(data).then(res => {
    //     if (res.code === '200') {
    //       const treeData = res.data.map(daItem => {
    //         return {
    //           id: daItem[id],
    ...
    handleLoorTabClick (e) {
      if (e.name === '2' && this.deviceList.tableData.length === 0) {
        this.onDeviceListSearch();
      }
      if (e.name === '3' && this.historyList.tableData.length === 0) {
        this.onHistoryListSearch();
      }
      if (e.name === '4' && this.logList.tableData.length === 0) {
        this.onLogListSearch();
      }
    },
    handleNormalTabClick (e) {
    },
    ...
    onPlayHistoryListCurrentChange (current) {
      this.playHistoryList.page.current = current;
      this._getSmokeDetectorWorkOrderForPage();
    },
    onPlayHistoryListPageSizeChange (pageSize) {
      this.historyList.page.pageSize = pageSize;
      this.onPlayHistoryListCurrentChange(1);
    },
    onPlayHistoryListSelectionChange () {

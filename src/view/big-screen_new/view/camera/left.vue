<template>
  <div id="cameraLeft" :class="{'componentView': true, 'show': show}">
    <div class="part_1">
      <ChartTitle partTitle="摄像头今日事件类型统计"/>
      <Event />
    </div>
    <div class="parat_2">
      <ChartTitle partTitle="摄像头告警类型统计"/>
      <warnType />
    </div>
  </div>
</template>
...
import Event from '../../components/camera/events'
// 告警类型统计
import warnType from '../../components/camera/warnType'
export default {
  data () {
    return {
      screenEnergyConsump: {},
  ...
  methods: {
    ...
    init2 () {
      ...
      let chartData = [
        ...
        [
          '故障率',
          parseInt(this.bigScreenNew.screenEnergyConsump.failureRate)
        ]
      ];
      option = {
        ...this.chart1Options,

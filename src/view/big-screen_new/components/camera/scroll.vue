...
export default {
  ...
  methods: {
    getData() {
      let modeTableData = getLocal("bigScreenNew")["cameraAlarmList"]["data"];
      if (modeTableData && modeTableData.length) {
        modeTableData.map((item) => {
          this.tableData.push([
            `<div style='width: 100%;height: 100%;padding: 10px 0' v-if="${item.alarmPicURL}">
              <img src=${item.alarmPicURL} style='width: 80%;height: 80%'></img>
            </div>`,
            item.cameraName,
            item.alarmTypeName,
            item.alarmTime,

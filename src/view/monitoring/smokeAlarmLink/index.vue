<template>
  <div id="smokeAlarmLink">
    <el-breadcrumb separator="/">
      <el-breadcrumb-item :to="{path: '/monitoring/securityMonitoring'}">安防监控</el-breadcrumb-item>
      <el-breadcrumb-item>摄像头事件详情</el-breadcrumb-item>
    </el-breadcrumb>
      <dv-loading v-show="!initShow"></dv-loading>
      <el-row :gutter="10" v-show="initShow">
          <el-col :span="8">
              <div class="flexCloumn list">
                <card class="flexCloumn" title="监控事件">
                    <search-form
                        ...
                        <div class="flexRow listItem" :class="alarmItem.alarmCode === item.alarmCode ? 'active' : ''">
                            ...
                            <div class="flexCloumn info">
                                ...
                                <div class="flexRow time">
                                  <el-tooltip :content="item.address" placement="top" effect="light">
...
export default {
  ...
  created () {
    // 获取路由中的参数并初始化视频地址
    const routeParams = this.$route.query ? this.$route.query : null
    if (routeParams) {
      this.alarmItem.alarmCode = routeParams.alarmCode
      this.alarmItem.serialNumber = routeParams.serialNumber
      this.alarmItem.deviceBindCode = routeParams.serialNumber
      this.searchVal.deviceName = routeParams.cameraName
      this.searchItems[2].initValue = routeParams.cameraName
      let parames = {
        imei: '',
        streetCode: '',
  ...
  methods: {
    async init () {
      ...
      this.list = [];
      this.currentIndex = 0;
      this.src = `https://open.ys7.com/ezopen/h5/iframe_se?url=ezopen://RPUENL@open.ys7.com/E63112920/1.live&accessToken=${this.common.accessToken}&templete=2`;
      await this.load();
      this.initShow = true;
      this.player = new EZUIKit.EZUIPlayer('Detailysopen');
      this.player.play();
      this.$forceUpdate();
    },
    resize () {

  ...
  <div class="warm-map-box">
    <div class="map">
      ...
      >
        ...
        <leftComponent />
        <rightComponent :serialNumber="serialNumber" />
        <centerComponent />
      </baidu-map>
    </div>
  </div>
...
import { getCameraMap } from "@/api/wisdomMonitor";
...
export default {
  ...
  methods: {
    async handler({ BMap, map }) {
      ...
      // });
      const infos = await this._getCameraMap();
      if (infos.length > 0) {
    ...
    _getCameraMap() {
      let query = {};
      return getCameraMap(query).then(async res => {
        if (res.code === "200") {
          const { data } = res;

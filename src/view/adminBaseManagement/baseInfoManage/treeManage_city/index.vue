...
export default {
  ...
  methods: {
    ...
    async loadNode (node, resolve) {
      ...
      if (node.level === 0) {
        const tree0 = await this._getTerminalTree({
          ...
          'city': '',
          'imei': ''
        }, 'city', 'city', 'city', {data: {optionsData: []}}, {parentData: []}, resolve);
      ...
      if (node.level === 1) {
        ...
        }, 'area', 'area', 'area', node, node.parent.data, resolve);
      ...
      if (node.level === 2) {
        ...
        }, 'address', 'address', 'address', node, node.parent.data, resolve);
    ...
    async _getTerminalTree (data, id, label, type, parentData, parentOptionData, resolve) {
      let treeData = [];
      ...
      await getTerminalTree({
        ...
        if (res.code === '200') {
          const optionsData = res.data.map(daItem => {
            return {
              value: daItem['streetCode'] || daItem['cityCode'] || daItem['areaCode'],
              label: daItem[label]
            }
          });
          // 当前层级的所有节点
          ...
          treeData = res.data.map(daItem => {
            ...
            return {
              id: daItem['streetCode'] || daItem['cityCode'] || daItem['areaCode'],
              label: daItem[label],
              type: type,
    ...
    nodeClick (data) {
      ...
      switch(data.type) {
        case 'city':
          parentNodeCode = data.cityCode;
          break;
        case 'area':
          parentNodeCode = data.areaCode;
          break;
        case 'address':
          parentNodeCode = data.streetCode;
          break;
        case 'imei': 
          parentNodeCode = data.imei;
          break;
        default:
          break;
      }

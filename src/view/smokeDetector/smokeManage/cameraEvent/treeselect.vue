...
export default {
    ...
    currentVal: {
      get () {
        if (this.value instanceof Array) {
          ...
          const val = this.value.length > 0 ? this.value.filter(v => {
            if (!!v.nodeCode && !reg.test(v.nodeCode) && v.nodeLevel === 3) {
              return v
            }
          }) : null;
          this.setDom(val);
      ...
      set (newVal) {
        if (newVal instanceof Array) {
          const reg = /[\u4e00-\u9fa5]/g;
          const val = newVal.filter(v => (!!v.nodeCode && !reg.test(v.nodeCode)) && v.nodeLevel === 3);
  ...
  methods: {
    ...
    loadAll (parames) {
      return getParkDeviceTree({...parames}).then(async res => {
        if (res.code === '200') {
          let dataList = [];
          res.data.map(async data => {
            ...
            if (data.nodeLevel !== 3) {
              ...
              addressChildren = await this.loadAll({
                parentNodeCode: data.nodeCode
              });
            } else {
              addressChildren = undefined
            }
            const id = data.nodeCode;
            const label = data.nodeName;
            dataList.push({
              ...parames,
              ...data,
              id,

...
<template>
  <pageMain idName='lightPoleDetail' :currentTitleList="type !== 'template' ? currentTitleList : []">
    <el-form
      ref="formData"
      id="formItem"
      :model="form"
      :label-width="'65px'"
    >
    <card title="基本信息" v-if="type !== 'template'">
      <FormItemText
        :layout="baseItems"
        labelWidth="0px"
      >
    ...
    <card title="控制策略" v-if="type !== 'template'">
      <div class="flexRow policyContainer">
        <formItem
          :formItems="formItems"
          :form="form"
          v-if="false"
        />
        <el-tabs v-model="activePolicy" @tab-click="handlePolicyClick">
          <el-tab-pane label="照明策略" name="light">
            <div class="lightPolicy">
              <div
                class="flexRow"
                :style="{marginBottom: '0.2rem'}"
                :class="`policyItems${form.lampGroup && form.lampGroup.length}`"
              >
                <div
                  class="flexCloumn flexCenter main"
                  v-for="(item, index) in form.lampGroup"
                  :key="item.groupNo"
                  v-if="index < 5"
                >
                  ...
                  <div
                    class="policyTitle"
                    @click="() => goLampGroupDetail(item, 'light')"
                  >
                    <el-tooltip class="formItem value" effect="light" placement="top-start">
                      <div slot="content">{{item.groupName}}</div>
                      <div>{{item.groupName}}</div>
                    </el-tooltip>
                    <div>{{`${item.workTime}~${item.closeTime}`}}</div>
                  </div>
                </div>
              </div>
              <div
              class="more"
              v-if="form.lampGroup && form.lampGroup.length > 5"
              @click="goMorePolicy('light')"
              >
              更多
              </div>
              <el-button
                ...
                class="button"
                title="设置此灯控制策略"
                @click.prevent.stop="handleSetPolicy"
              >
                设置此灯控制策略
              </el-button>
            </div>
          </el-tab-pane>
          <el-tab-pane label="LED策略" name="led">
            <div class="ledPolicy">
              <div
                class="flexRow"
                :style="{marginBottom: '0.2rem'}"
                :class="`policyItems${form.ledGroup && form.ledGroup.length}`"
              >
                <div
                  class="flexCloumn flexCenter main"
                  v-for="(item, index) in form.ledGroup"
                  :key="item.groupNo"
                >
                  <span class="number">{{index + 1}}</span>
                  <div
                    class="policyTitle"
                    @click="() => goLampGroupDetail(item, 'led')"
                  >
                    <el-tooltip class="formItem value" effect="light" placement="top-start">
                      <div slot="content">{{item.groupName}}</div>
                      <div>{{item.groupName}}</div>
              ...
              <div
              class="more"
              v-if="form.ledGroup && form.ledGroup.length > 5"
              @click="goMorePolicy('led')"
              >
              更多
              </div>
            </div>
          </el-tab-pane>
          <el-tab-pane label="广播策略" name="video">
            <div class="videoPolicy">
              <div
                class="flexRow"
                :style="{marginBottom: '0.2rem'}"
                :class="`policyItems${form.broadcastTeam && form.broadcastTeam.length}`"
              >
                <div
                  class="flexCloumn flexCenter main"
                  v-for="(item, index) in form.broadcastTeam"
                  :key="item.teamNo"
                >
                  <span class="number">{{index + 1}}</span>
                  <div
                    class="policyTitle"
                    @click="() => goLampGroupDetail(item, 'video')"
                  >
                    <el-tooltip class="formItem value" effect="light" placement="top-start">
                      <div slot="content">{{item.teamName}}</div>
              ...
              <div
              class="more"
              v-if="form.broadcastTeam && form.broadcastTeam.length > 5"
              @click="goMorePolicy('video')"
              >
              更多
              </div>
            </div>
    ...
    <card title="" class="realTimeInfo">
      <div class="controlBtn" slot:titleOther>
        <el-button type="primary" @click="goLongRangControl">远程控制</el-button>
      </div>
...
import watchImg from './images/img2.png';

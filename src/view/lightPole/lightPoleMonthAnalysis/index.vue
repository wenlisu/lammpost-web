...
<template>
  <div id='lightPoleMonthAnalysis'>
    <!-- 智慧灯杆月分析 Monthly analysis -->
    <header-title icon="iconfont icon-shouye" title="智慧灯杆月分析">
      <search-form
        slot="searchBox"
...
export default {
  ...
  data () {
    return {
      ...
      },
      {
        ...
        sortable: true,
        render: (h, scope) => {
          // let value = parseFloat(scope.row.isOnRecordsRate)
  ...
  methods: {
    exportInfo() {
      exportConsumptionAnalysis(2).then(res => {
        let blob = new Blob([res.data]);
        let url = window.URL.createObjectURL(blob);
        download(url, "灯杆能耗(月).xls");
      })
    },
    formatRecordsRate (row, column) {
      return column
    },
    getRadiosType (type) {
    ...
    onSearch (val) {
      let parames = {
        ...val,
        startDate: val.monthDate && (val.monthDate[0] || ''),
        endDate: val.monthDate && (val.monthDate[1] || '')
      }
      delete parames.monthDate;
    ...
    getDataAllList () {
      ...
      lampMonthlyStatistics(data).then(res => {
        if (res.code === '200') {
          const {
            data,
            total

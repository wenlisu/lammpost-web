<template>
  <div id="adminFormItem">
    ...
    <el-select
      ...
      v-bind="bindProps"
      v-on="bindEvents"
      :placeholder="item.props.placeholder ? item.props.placeholder : `请选择${item.label}`"
      clearable
      value-key="name"
      class="filter-item-select"
      :style="{maxWidth:item.props.width||'140px'}"
      :disabled="item.props.disabled"
      @change="((value)=>{onChange(value, item.onChange)})"
      v-if="item.type==='select'"
    >
      <el-option
        v-for="(optionItem, index) in [{id: '-1', name: `所有${item.label}`}, ...item.props.options]"
        :key="`select${index}${item.props.fieldName}${optionItem.value}`"
        :label="optionItem.name || optionItem.text"
        :value="item.obj ? optionItem : `${optionItem.id}` || `${optionItem.value}`"
      />
    </el-select>
    <!-- 单选 -->
    <el-radio-group
    ...
    <!-- 树结构选择框 -->
      ...
      :normalizer="(node) => {return formatterNode(node, item.normalizer)}"
      :load-options="({action, parentNode, callback}) => {return loadOptions(action, parentNode, callback, item.callback)}"
      :placeholder="item.props.placeholder ? item.props.placeholder : `请选择${item.label}`"
      v-model="currentVal"
      />
  </div>

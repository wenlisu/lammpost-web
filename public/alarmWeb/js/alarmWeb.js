...
function dbClickDevice(obj){
   ...
   var id = $(obj)[0].id;
   var deviceId = id.split("_")[1];
   postData('/api/iot/alarmCall/getAlarmCallDtl', {serialNumber: deviceId}).then(function(response) {
	if (response.code === '200') {
		if($('#play_'+deviceId)[0]) return;
		$(".liSelected").removeClass('liSelected');
		$(obj).addClass("liSelected");
		playVideo(deviceId,false, response.data);
	} else {
...
//播放视频
function playVideo(deviceId, isTalk, data){
   var alarmCount = 0;
   for(var i = 0; i< $("video").length; i++){
   ...
   var html = '<div class="videoboxDiv" ondblclick="launchFullscreen(this)">'+
					'<video id="play_'+deviceId+'" onclick="selectedVideo(this)" oncanplay="canplayVideo(this)"></video>' +
					'<div class="videoInfo">' +
						'<div class="icon"><img src="./image/address.png"/></div>' +
						'<div class="titleInfo"><span>'+$('#device_'+deviceId).text()+'</span><span>' + data.address +'</span></div>' +
						'<div class="btnDiv">' +
						'<div id="monitorBtn">' +
							'<i class="iconfont icon-jiankongyunwei"></i>监控视频' +
						'</div>' +
						'<div id="linkBtn">' +
							'<i class="iconfont icon-churujilu"></i>场景联动' +
						'</div>' +
					'</div>' +
					'</div>'+
	...
	if(!$('#play_'+deviceId)[0]) {
		...
		$('#linkBtn').click(function() {
			window.parent['goSmokeAlarmLink']();
		});
		// 监控视频
		$('#monitorBtn').click(function() {
			var params = JSON.parse(JSON.stringify(data))
			window.parent['getMonitorVideo'](params);
		});
	}
	if(!$(".selectVideo")[0]){
		...
		for(var i =0; i<groupDevices.length;i++){
			...
			var html = '<div class="videoboxDiv" ondblclick="launchFullscreen(this)">'+
					...
					'<div class="videoInfo">' +
						...
						'<div class="titleInfo"><span>'+$('#device_'+deviceId).text()+'</span><span>' + data.address +'</span></div>' +
						'<div class="btnDiv">' +
						'<div id="monitorBtn" onclick="getMonitor">' +
							'<i class="iconfont icon-jiankongyunwei"></i>监控视频' +
						'</div>' +
						'<div id="linkBtn">' +
							'<i class="iconfont icon-churujilu"></i>场景联动' +
...
function ondbclick(row){
	...
	if($("#"+id+" .alarmStatus").hasClass('dealing')){
		...
		postData('/api/iot/alarmCall/getAlarmCallDtl', {serialNumber: deviceId}).then(function(response) {
			if (response.code === '200') {
				playVideo(deviceId,true, response.data);
				setTimeout(() => {
